...
// LOADS functions.php first, then this is included.
...
//  function property_overview_image($args = '') {
    ...
    if(!empty($image)) {
      ob_start();
      ?>
      <div class="property_image" data="local"><a href="<?php echo $thumbnail_link; ?>" title="<?php echo $property['post_title'] . ($property['parent_title'] ? __(' of ', 'wpp') . $property['parent_title'] : "");?>"  class="property_overview_thumb property_overview_thumb_<?php echo $thumbnail_size; ?> <?php echo $link_class; ?> thumbnail" rel="properties" ><img width="<?php echo $image['width']; ?>" height="<?php echo $image['height']; ?>" src="<?php echo $image['link']; ?>" alt="<?php echo $property['post_title'];?>" style="width:<?php echo $image['width']; ?>px;height:<?php echo $image['height']; ?>px;" /></a></div>
      <?php
    ...
    switch($use_input_type) {
      ...
      case 'range_dropdown':
        ...
        case 'dropdown':
          ...
          <option value="-1"><?php _e( 'Any Category' ,'wpp' ) ?></option>
          <?php 
          if(taxonomy_exists($search_values[$attrib][0])){
			  $tax_terms = get_terms($search_values[$attrib]);
			  unset($search_values[$attrib][0]); //have to eliminate it from the list..
			  foreach($tax_terms as $term){
				  $search_values[$attrib][] = $term;
			  }
		  }
          foreach( $search_values[$attrib] as $v ) : ?>
            <option value="<?php echo esc_attr($v->slug); ?>" <?php selected($value, $v->slug ); ?>><?php echo esc_attr( apply_filters("wpp_stat_filter_{$attrib}", $v->name	) ); ?></option>
          <?php endforeach; ?>
          </select>
          <?php
          break;
